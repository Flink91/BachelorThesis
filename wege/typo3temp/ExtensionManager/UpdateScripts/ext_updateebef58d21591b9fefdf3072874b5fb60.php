 ...
 *
 */
class ext_updateebef58d21591b9fefdf3072874b5fb60 {
    ...
    protected function updateFileRelations()
    {
        ...
        $countFilesWithParent = $GLOBALS['TYPO3_DB']->exec_SELECTcountRows('*', 'tx_news_domain_model_file',
            'deleted=0 AND parent != 0');
        if ($countFilesWithParent === 0 && $countNewsWithFileRelation > 0) {
            $newsCount = 0;
            // select news with related files
            $newsQuery = $GLOBALS['TYPO3_DB']->exec_SELECTquery('uid,related_files', 'tx_news_domain_model_news',
                'deleted=0 AND related_files !=""');
